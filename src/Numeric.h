...
public:
	...
	double col(size_t n, size_t i) {
		...
		if (!lc.colValid) {
			...
			int ts;
			lc.col.clear();
			lc.col.push_back(data.getNumber(s, value, dp, ts, data.calc.unitIsMinutes) ? value : std::numeric_limits<double>::quiet_NaN());
			for (size_t j = 0; j = s.find_first_not_of("\t ", j), j != std::string::npos;) {
				size_t k = s.find_first_of("\t ", j);
				lc.col.push_back(data.getNumber(s.substr(j, k - j), value, dp, ts, data.calc.unitIsMinutes) ? value : std::numeric_limits<double>::quiet_NaN());
	...
	double reg(size_t n, size_t i, Scintilla::Position* response = 0) {
		...
		if (!lc.regValid || i > lc.reg.size()) {
			...
			if (found < 0) for (size_t j = 0; j <= regCount; ++j) lc.reg.push_back(std::numeric_limits<double>::quiet_NaN());
				...
				size_t dp;
				int ts;
				lc.reg.push_back( data.getNumber(data.sci.TargetText(), value, dp, ts, data.calc.unitIsMinutes)
					            ? value : std::numeric_limits<double>::quiet_NaN() );
				for (size_t j = 1; j <= regCount; ++j)
				    lc.reg.push_back( data.getNumber(data.sci.Tag(static_cast<int>(j)), value, dp, ts, data.calc.unitIsMinutes)
						            ? value : std::numeric_limits<double>::quiet_NaN() );
	...
	double tab(size_t n, size_t i) {
		...
		if (!lc.tabValid) {
			...
			int ts;
			lc.tab.clear();
			lc.tab.push_back(data.getNumber(s, value, dp, ts, data.calc.unitIsMinutes) ? value : std::numeric_limits<double>::quiet_NaN());
			for (size_t j = 0; j < s.length();) {
				size_t k = s.find_first_of("\t", j);
				lc.tab.push_back(data.getNumber(s.substr(j, k - j), value, dp, ts, data.calc.unitIsMinutes) ? value : std::numeric_limits<double>::quiet_NaN());
				if (k == std::string::npos) break;
				j = k + 1;
			}
		}
